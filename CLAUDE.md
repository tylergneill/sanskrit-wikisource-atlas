...
**Phase 2 (page-metadata fetch) is currently dominated by this penalty, not by real work.** Analysis of one full run's `.api_cache/` file timestamps showed a strikingly regular pattern: exactly 10 `prop=revisions` batch requests land in quick succession (~0.5-1s apart, matching the default `--delay 0.5`), then Envoy trips and the run stalls for ~45-95s (typically ~55s) waiting out `Retry-After`, then repeats. Over that full run, **over 90% of phase-2 wall-clock time was spent in these stalls**, not fetching. This is a hard, consistent threshold (10 requests), not noisy — but no safe steady-state pace that avoids tripping it has been empirically found yet (that would require live binary-search testing against the real API, which itself risks tripping the limiter — deliberately not done as of this writing). `scrape.py` exposes `--page-meta-delay` as a separate, independently tunable delay just for this phase-2 loop (defaults to `--delay` if unset) — the knob exists for future tuning; nothing has been calibrated through it yet.
...
## Implemented but DISABLED BY DEFAULT: scraper chasing MediaWiki subpages (the "conservatism problem")

**Status: the code exists, is wired up, and was validated against one isolated test case — but is gated behind `--recurse-subpages` (default off) because it reliably trips Wikimedia's rate limiter on real, deeply-nested works.** `recurse_subpages` defaults to `False` in `build_skeleton()`'s signature; a normal `make scrape` / `python scrape.py --verbose` run does **not** exercise this path at all. Do not assume subpage descent is happening in the current `docs/data/tree.json` unless the scraper was explicitly run with `--recurse-subpages`.
...
**Implemented** (roadmap item #2, "descend more levels"): `fetch_subpages()` queries `list=allpages&apprefix=<title>/&apnamespace=0` for a given page title, and `collect_subpages_recursive()` recursively follows discovered subpages to any nesting depth (e.g. `Title/Part1/Section2`), guarded by a `seen_titles` set. When `recurse_subpages=True`, `build_skeleton()` calls this for every page found via `categorymembers` and flattens the results in as ordinary sibling pages in the same category node (not nested under the index page as a distinct node type — that's deferred to roadmap item #3, consolidation). Validated in isolation against नैषधीयचरितम्: all 22 सर्गः subpages (plus a misspelled duplicate, अष्ठमः सर्गः) appeared correctly.

**Why it's disabled by default:** per an in-code comment at `scrape.py:430-435`, enabling this was confirmed to repeatedly trip Wikimedia's rate limiter on real deeply-nested works — बैबल् (Bible) is named specifically, which nests book/chapter subpages several levels deep — even with the `SUBPAGE_PROBE_MAX_BYTES` size gate and normal request pacing in place. This is a broader/harder case than the नैषधीयचरितम् validation above (2 levels deep, 22 subpages) and was NOT resolved before the work was shelved. The code is deliberately kept (not ripped out) behind the `--recurse-subpages` flag for future work, pending more robust request-volume handling — see roadmap item #2 in the pipeline upgrade plan memory. **Do not re-enable by default or claim this is "done" without addressing that failure case first.**

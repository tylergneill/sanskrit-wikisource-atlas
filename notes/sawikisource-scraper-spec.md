...
- **Scope of the single XML file**: all namespaces in one export, including Main, Category, Index, Template, and Module — one download covers every input the pipeline needs. **Page namespace is deliberately not used** (see Transclusion below), even though it's present in the dump.
...
| Index (104)\* | Per-work scan metadata; surfaced only when untranscluded |
...
## Display abstraction
Category tree is the primary browsing structure. Top-level (Index/root) nodes show a summary — total size, latest change date — by default; finer subpage-level detail is available but collapsed. Untranscluded Index items appear as leaf entries (not expandable into Page-level detail, since Page-namespace content isn't fetched).

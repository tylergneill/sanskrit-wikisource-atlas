...
# writing). Each step is a separate `python -m pipeline.backfill --months
# OLDER NEWER` invocation, so progress/output is visible per-step and a
# failure on one month doesn't lose earlier progress (already-appended
# changelog entries and already-fetched dumps are skipped/reused on rerun --
# see ensure_month/ensure_snapshot in pipeline/backfill.py).
#
...
MONTHS=$(python3 -m pipeline.fetch_legacy --list | cut -f1 | sed 's/$/-01/' | awk -v cutover="$NEWEST_ANCHORED" '$0 < cutover')
if [[ -z "$MONTHS" ]]; then
